    ...
    return (
        <div className="detail-rune-info">
            <div className={`detail-rune-description ${viewRune[index] ? 'show' : ''}`}>
                <span dangerouslySetInnerHTML={ {__html: runeType === "main" ? rune.longDesc : rune.name} }></span>
            </div>
            <img
...
const DetailRuneInfo = ({player, viewMainRune, viewSubRune, setViewMainRune, setViewSubRune, index, getMainRune, getSubRune}) => {
    return (
        <>
            {player.perks.styles.map((perk, perkIndex) => {
                if (perkIndex === 0) {
                    return <RuneInfo runeType="main" getRune={getMainRune} perk={perk} index={index} viewRune={viewMainRune} setViewRune={setViewMainRune} />
                } else if (perkIndex === 1) {
                    return <RuneInfo runeType="sub" getRune={getSubRune} perk={perk} index={index} viewRune={viewSubRune} setViewRune={setViewSubRune} />
                }
            })}
        </>
    );
